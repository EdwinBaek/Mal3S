...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model1):
        ...
        torch.save(model1.state_dict(), './Checkpoint/Multi_4F_ft-%s_hc-%s_oc-%s_hp-%s_op-%s_loss-%s_checkpoint.pt' % (fastText_size, self.hyperparameter[0], self.hyperparameter[1], self.hyperparameter[2], self.hyperparameter[3], self.hyperparameter[4]))
...
class Mal3S(nn.Module):
    def __init__(self, in_channel, hidden_channel, out_channel, output_pooling_size, num_class):
        ...
        self.single_opcode_conv_layer = single_feature_conv(in_channel, hidden_channel, out_channel, output_pooling_size)
        self.single_apicall_conv_layer = single_feature_conv(in_channel, hidden_channel, out_channel, output_pooling_size)
        self.single_string_conv_layer = single_feature_conv(in_channel, hidden_channel, out_channel, output_pooling_size)
        self.single_dllapi_conv_layer = single_feature_conv(in_channel, hidden_channel, out_channel, output_pooling_size)

        concat_cells = sum([i * i for i in output_pooling_size]) * out_channel * 4
    ...
    def forward(self, opcode, apicall, string, dllapi):
        single_opcode_out = self.single_opcode_conv_layer(opcode)
        single_apicall_out = self.single_apicall_conv_layer(apicall)
        single_string_out = self.single_string_conv_layer(string)
        single_dllapi_out = self.single_dllapi_conv_layer(dllapi)
        # print('Mal3D_single_feature_out shape >>> ', single_feature_out.shape, '\n')

        multi_feature_out = torch.cat([single_opcode_out, single_apicall_out, single_string_out, single_apicall_out], 1)
        out = self.dense_layer_1(multi_feature_out)
...
if __name__ == '__main__':
    ...
    summary(model, [(in_channels, image_height, image_width), (in_channels, image_height, image_width), (in_channels, image_height, image_width), (in_channels, image_height, image_width)])
    ...
    print('Train dataset size * 4 : ', len(train_dataloader))
    print('Valid dataset size * 4 : ', len(valid_dataloader))
    print('Test dataset size * 4 : ', len(test_dataloader))
    print('----------------------------')
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (image, label) in enumerate(train_dataloader):
            ...
            if train_image_cnt == 4:
                train_cnt += 1
                optimizer.zero_grad()
                output = model(opcode=train_images[0], apicall=train_images[1], string=train_images[2], dllapi=train_images[3]).to(device)
        ...
        train_acc = train_true / (len(train_dataloader.dataset) / 4)
        ...
        with torch.no_grad():
            for batch_idx, (image, label) in enumerate(valid_dataloader):
                ...
                valid_image_cnt += 1
                if valid_image_cnt == 4:
                    valid_cnt += 1
                    output = model(opcode=valid_images[0], apicall=valid_images[1], string=valid_images[2], dllapi=valid_images[3]).to(device)
                    pred = torch.max(output, 1)[1]
                    loss = criterion(output, label)
            ...
            valid_acc = valid_true / (len(valid_dataloader.dataset) / 4)
    ...
    # Test
    model.load_state_dict(torch.load('./Checkpoint/Multi_4F_ft-%s_hc-%s_oc-%s_hp-%s_op-%s_loss-%s_checkpoint.pt' % (
    fastText_size, hyperparameter[0], hyperparameter[1], hyperparameter[2], hyperparameter[3], hyperparameter[4])))
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (image, label) in enumerate(test_dataloader):
            ...
            if test_image_cnt == 4:
                test_cnt += 1
                output = model(opcode=test_images[0], apicall=test_images[1], string=test_images[2], dllapi=test_images[3]).to(device)
                pred = torch.max(output, 1)[1]
                loss = criterion(output, label)
        ...
        test_acc = test_true / (len(test_dataloader.dataset) / 4)
    ...
    label_name = ['Benign', 'Ramnit', 'Lollipop', 'Kelihos_ver3', 'Vundo', 'Simda', 'Tracur', 'Kelihos_ver1',
    ...
    for body_idx in range(len(cm)):
        print_test_result.append(([label_name[body_idx], cm[body_idx][0], cm[body_idx][1], cm[body_idx][2],
                                   cm[body_idx][3], cm[body_idx][4], cm[body_idx][5], cm[body_idx][6], cm[body_idx][7],
                                   cm[body_idx][8], cm[body_idx][9]]))
    ...
    with open('./Result/Multi_4F_ft-%s_hc-%s_oc-%s_hp-%s_op-%s_loss-%s_test_result.csv' % (
    fastText_size, hyperparameter[0], hyperparameter[1], hyperparameter[2], hyperparameter[3], hyperparameter[4]), 'w', newline='') as file:
    ...
    for i in range(epochs):
        print_train_valid_result.append(
            [(i + 1), recode_train_result[i][0], recode_train_result[i][1], recode_valid_result[i][0],
             recode_valid_result[i][1]])

    with open('./Result/Multi_4F_ft-%s_hc-%s_oc-%s_hp-%s_op-%s_loss-%s_train_valid_recode.csv' % (
    fastText_size, hyperparameter[0], hyperparameter[1], hyperparameter[2], hyperparameter[3], hyperparameter[4]), 'w', newline='') as file:
        write = csv.writer(file)
