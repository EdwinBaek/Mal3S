...
GPU_NUM = 1  # Enter the desired GPU number
...
class MalD3S_Dataset(Dataset):
    def __init__(self, mode, transform=None):
        ...
        if mode == 'train':
            ...
            for idx in range(len(self.byte_image)):
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model1):
        ...
        torch.save(model1.state_dict(), './Checkpoint/MSPPNet_ft-%s_hc-%s_oc-%s_hp-%s_op-%s_loss-%s_checkpoint.pt' % (fastText_size, self.hyperparameter[0], self.hyperparameter[1], self.hyperparameter[2], self.hyperparameter[3], self.hyperparameter[4]))
...
class multi_feature_conv(nn.Module):
    def __init__(self, in_channel, hidden_channel, out_channel, hidden_pooling_size, output_pooling_size):
        super(multi_feature_conv, self).__init__()
        self.hidden_pooling_size = hidden_pooling_size  # singe list (ex; [16])
        self.output_pooling_size = output_pooling_size  # multi list (ex; [4, 2, 1])
        self.opcode_conv = nn.Conv2d(in_channel, hidden_channel, kernel_size=3, stride=1, padding=(512, 512))
        self.apicall_conv = nn.Conv2d(in_channel, hidden_channel, kernel_size=3, stride=1, padding=(512, 512))
        self.string_conv = nn.Conv2d(in_channel, hidden_channel, kernel_size=3, stride=1, padding=(512, 512))
        self.dllapi_conv = nn.Conv2d(in_channel, hidden_channel, kernel_size=3, stride=1, padding=(512, 512))

        self.concat_conv = nn.Conv2d(hidden_channel * 4, out_channel, kernel_size=3, stride=1)

    def forward(self, opcode, apicall, string, dllapi):
        opcode_out = F.relu(self.opcode_conv(opcode))
        # print('multi_opcode_out shape >>> ', opcode_out.shape, '\n')

        apicall_out = F.relu(self.apicall_conv(apicall))
        # print('multi_apicall_out shape >>> ', apicall_out.shape, '\n')

        string_out = F.relu(self.string_conv(string))
        # print('multi_string_out shape >>> ', string_out.shape, '\n')

        dllapi_out = F.relu(self.dllapi_conv(dllapi))
        # print('multi_dllapi_out shape >>> ', dllapi_out.shape, '\n')

        opcode_out = hidden_pooling(prev_conv=opcode_out,
                                    prev_conv_size=[int(opcode_out.size(2)), int(opcode_out.size(3))],
                                    hidden_pool_size=self.hidden_pooling_size)
        # print('multi_opcode_out pooling shape >>> ', opcode_out.shape, '\n')

        apicall_out = hidden_pooling(prev_conv=apicall_out,
                                     prev_conv_size=[int(apicall_out.size(2)), int(apicall_out.size(3))],
                                     hidden_pool_size=self.hidden_pooling_size)
        # print('multi_apicall_out pooling shape >>> ', apicall_out.shape, '\n')

        string_out = hidden_pooling(prev_conv=string_out,
                                    prev_conv_size=[int(string_out.size(2)), int(string_out.size(3))],
                                    hidden_pool_size=self.hidden_pooling_size)
        # print('multi_dllstring_out pooling shape >>> ', string_out.shape, '\n')

        dllapi_out = hidden_pooling(prev_conv=dllapi_out,
                                    prev_conv_size=[int(dllapi_out.size(2)), int(dllapi_out.size(3))],
                                    hidden_pool_size=self.hidden_pooling_size)
        # print('multi_dllapi_out pooling shape >>> ', dllapi_out.shape, '\n')

        feature_map = torch.cat([opcode_out, apicall_out, string_out, dllapi_out], 1)
        # print('multi_feature_map shape >>> ', feature_map.shape, '\n')

        out = F.relu((self.concat_conv(feature_map)))
        # print('multi_feature_map conv shape >>> ', out.shape, '\n')

        out = spatial_pyramid_pooling(prev_conv=out,
                                      num_sample=out.size(0),
                                      prev_conv_size=[int(out.size(2)), int(out.size(3))],
                                      out_pool_size=self.output_pooling_size)
        # print('multi feature conv_out shape >>> ', out.shape, '\n')
        return out
...
class single_feature_conv(nn.Module):
    def __init__(self, in_channel, hidden_channel, out_channel, output_pooling_size):
        ...
        self.byte_conv_1 = nn.Conv2d(in_channel, hidden_channel//2, kernel_size=3, stride=1, padding=(512, 512))
...
class Mal3S(nn.Module):
    def __init__(self, in_channel, hidden_channel, out_channel, hidden_pooling_size, output_pooling_size, num_class):
        super(Mal3S, self).__init__()
        self.multi_feature_conv_layer = multi_feature_conv(in_channel, hidden_channel, out_channel, hidden_pooling_size, output_pooling_size)
        self.single_feature_conv_layer = single_feature_conv(in_channel, hidden_channel, out_channel, output_pooling_size)

        concat_cells = sum([i * i for i in output_pooling_size]) * out_channel * 2
        # print('concat_cells size >>> ', concat_cells)

        self.dense_layer_1 = nn.Linear(concat_cells, concat_cells//2)
        self.dense_layer_2 = nn.Linear(concat_cells//2, concat_cells//4)
        self.output_layer = nn.Linear(concat_cells//4, num_class)

    def forward(self, opcode, apicall, string, dllapi, byte):
        multi_feature_out = self.multi_feature_conv_layer(opcode, apicall, string, dllapi)
        # print('Mal3D_multi_feature_out shape >>> ', multi_feature_out.shape, '\n')

        single_feature_out = self.single_feature_conv_layer(byte)
        # print('Mal3D_single_feature_out shape >>> ', single_feature_out.shape, '\n')

        out = torch.cat([multi_feature_out, single_feature_out], 1)
        # print('Mal3D_concat_out shape >>> ', out.shape, '\n')

        out = self.dense_layer_1(out)
        # print('Mal3D_dense1_out shape >>> ', out.shape, '\n')

        out = self.dense_layer_2(out)
        # print('Mal3D_dense2_out shape >>> ', out.shape, '\n')
...
if __name__ == '__main__':
    ...
    # learning_rate = 1e-3
    patience = 20
    used_loss = 'final_shuffle_23'  # CrossEntropy(CE), Class Balanced Loss(CB)
    hyperparameter = [hidden_channels, out_channels, hidden_pooling_size[0], sum(output_pooling_size), used_loss]

    model = Mal3S(in_channels, hidden_channels, out_channels, hidden_pooling_size, output_pooling_size, num_classes).to(device)
    # model = nn.DataParallel(model)  # GPU 병렬 설정
    summary(model, [(in_channels, image_height, image_width), (in_channels, image_height, image_width), (in_channels, image_height, image_width), (in_channels, image_height, image_width), (in_channels, image_height, image_width)])
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=128)
    valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=128)
    test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=128)


    print('----------------------------')
    print('>>> Dataset information <<<')
    print('Train dataset size * 5 : ', len(train_dataloader))
    print('Valid dataset size * 5 : ', len(valid_dataloader))
    print('Test dataset size * 5 : ', len(test_dataloader))
    print('----------------------------')
    ...
    for epoch in range(epochs):
        ...
        train_image_cnt = 0  # 이미지 5개씩 묶기 위한 cnt
        train_images = []  # 이미지 5개씩 묶은 list
        for batch_idx, (image, label) in enumerate(train_dataloader):
            image, label = image.to(device), label.to(device)
            train_images.append(image)
            train_image_cnt += 1

            if train_image_cnt == 5:
                train_cnt += 1
                optimizer.zero_grad()
                output = model(byte=train_images[0], opcode=train_images[1], apicall=train_images[2], string=train_images[3], dllapi=train_images[4]).to(device)
        ...
        train_acc = train_true / (len(train_dataloader.dataset)/5)
        ...
        valid_image_cnt = 0  # 이미지 5개씩 묶기 위한 cnt
        valid_images = []  # 이미지 5개씩 묶은 list
        with torch.no_grad():
            for batch_idx, (image, label) in enumerate(valid_dataloader):
                image, label = image.to(device), label.to(device)
                valid_images.append(image)
                valid_image_cnt += 1
                if valid_image_cnt == 5:
                    valid_cnt += 1
                    output = model(byte=valid_images[0], opcode=valid_images[1], apicall=valid_images[2], string=valid_images[3], dllapi=valid_images[4]).to(device)
                    pred = torch.max(output, 1)[1]
                    loss = criterion(output, label)
            ...
            valid_acc = valid_true / (len(valid_dataloader.dataset)/5)
    ...
    # Test
    model.load_state_dict(torch.load('./Checkpoint/MSPPNet_ft-%s_hc-%s_oc-%s_hp-%s_op-%s_loss-%s_checkpoint.pt' % (fastText_size, hyperparameter[0], hyperparameter[1], hyperparameter[2], hyperparameter[3], hyperparameter[4])))
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (image, label) in enumerate(test_dataloader):
            ...
            if test_image_cnt == 5:
                test_cnt += 1
                output = model(byte=test_images[0], opcode=test_images[1], apicall=test_images[2], string=test_images[3], dllapi=test_images[4]).to(device)
                pred = torch.max(output, 1)[1]
                loss = criterion(output, label)
        ...
        test_acc = test_true / (len(test_dataloader.dataset)/5)
    ...
    test_file_name = []
    for data_Info in test_Data_Info:
        test_file_name.append(data_Info[0])
    ...
    test_recode = []
    test_recode.append(['file', 'actual', 'pred'])
    for i in range(len(test_raw_Label)):
        if test_raw_Label[i] != test_pred.detach().cpu().numpy()[i]:
            test_recode.append([test_file_name[i], test_raw_Label[i], test_pred.detach().cpu().numpy()[i]])
    ...
    with open('./Result/MSPPNet_ft-%s_hc-%s_oc-%s_hp-%s_op-%s_loss-%s_test_result.csv' % (fastText_size, hyperparameter[0], hyperparameter[1], hyperparameter[2], hyperparameter[3], hyperparameter[4]), 'w', newline='') as file:
    ...
    with open('./Result/MSPPNet_ft-%s_hc-%s_oc-%s_hp-%s_op-%s_loss-%s_train_valid_recode.csv' % (fastText_size, hyperparameter[0], hyperparameter[1], hyperparameter[2], hyperparameter[3],hyperparameter[4]), 'w', newline='') as file:
        write = csv.writer(file)
        write.writerows(print_train_valid_result)

    with open('./Result/test_recode_final_shuffle_23.csv', 'w', newline='') as file:
        write = csv.writer(file)
        write.writerows(test_recode)