...
def GenerateDllApiSet(asm_path, DllApiSet_dst_path):
        ...
        with io.open(os.path.join(asm_path, asm), 'r', encoding='ISO-8859-1') as f:
            ...
            for source_line in lines:
                ...
                try:
                    ...
                    if "dll" in result:
                        now_DLL = str(result.dll).lower()
                        if now_DLL.find('.dll') == -1:
                            now_DLL += '.dll'
                ...
                try:
                    ...
                    if "API" in result:
                        now_API = str(result.API).lower()
                        if now_API not in API_list:
                            API_list.append(now_API)
                        if now_API not in DllApi_dict[now_DLL]:
                            DllApi_dict[now_DLL].append(now_API)
                except Exception as e:
                    pass
    ...
    with open('./DLL_LIST.csv', 'wb') as total_dll:
        wr = csv.writer(total_dll)
        for dll_name in DLL_list:
            wr.writerow([dll_name])

    with open('./API_LIST.csv', 'wb') as total_api:
        wr = csv.writer(total_api)
        for api_name in API_list:
            wr.writerow([api_name])


# (NOT USE) Generate Total DLL and API list (concatenate ASM feature and PEfile feature)
def GenerateTotalDllApiList():
    with open('./DLL_LIST.txt', 'r') as asm_dll_ALL:
        line = asm_dll_ALL.readline()
        ASM_DLL_LIST = line.rstrip(' ').split(' ')
    with open('./API_LIST.txt', 'r') as asm_api_ALL:
        line = asm_api_ALL.readline()
        ASM_API_LIST = line.rstrip(' ').split(' ')
    with open('./Benign_DLL_LIST.txt', 'r') as pe_dll_ALL:
        line = pe_dll_ALL.readline()
        PE_DLL_LIST = line.rstrip(' ').split(' ')
    with open('./Benign_API_LIST.txt', 'r') as pe_api_ALL:
        line = pe_api_ALL.readline()
        PE_API_LIST = line.rstrip(' ').split(' ')

    TOTAL_API_LIST, TOTAL_DLL_LIST = [], []
    for dll_name in ASM_DLL_LIST:
        if dll_name not in TOTAL_DLL_LIST:
            TOTAL_DLL_LIST.append(dll_name)
    for dll_name in PE_DLL_LIST:
        if dll_name not in TOTAL_DLL_LIST:
            TOTAL_DLL_LIST.append(dll_name)

    for api_name in ASM_API_LIST:
        if api_name not in TOTAL_API_LIST:
            TOTAL_API_LIST.append(api_name)
    for api_name in PE_API_LIST:
        if api_name not in TOTAL_API_LIST:
            TOTAL_API_LIST.append(api_name)

    with open('./TOTAL_DLL_LIST.txt', 'w') as f_dll_ALL:
        for value in TOTAL_DLL_LIST:
            f_dll_ALL.write('%s ' % value)
    with open('./TOTAL_API_LIST.txt', 'w') as f_api_ALL:
        for value in TOTAL_API_LIST:
            f_api_ALL.write('%s ' % value)


# (before edit at 221024) Extract about DLL-API image of ASM files
...
#
#                             if (APIname in this_file_API_list) and (APIname in API_LIST):
#                                 for key, value in DllApi_dict.items():
#                                     if APIname in value:
...
def GenerateDllApiImg(asm_path, DllApiSet_dst_path, DllApiImage_dst_path):
    # (after edit at 221024) Extract about DLL-API image of ASM files
    DLL_LIST, API_LIST = [], []
        ...
        with open(os.path.join(DllApiSet_dst_path, txt_file), 'r') as f_txt:
            ...
            for line in lines:
                ...
                if dll_name in DLL_LIST:
                    API_names = API_names.lstrip(' ').rstrip('\n')
                    API_names = API_names.split(' ')
                    DllApi_dict[dll_name] = []
                    for api in API_names:
                        if api in TOTAL_API_LIST:
                            DllApi_dict[dll_name].append(api)
            ...
            for source_line in lines:
                ...
                try:
                    ...
                    if "api" in result:
                        ...
                        if APIname.find("ds:") != -1:
                            APIname = APIname.lstrip("ds:")
                        if APIname in TOTAL_API_LIST:
                            for key, value in DllApi_dict.items():
                                if APIname in value:
                                    try:
                                        y = DLL_LIST.index(key)
                                        x = API_LIST[y].index(APIname)
                                        if DLL_API_matrix[y][x] == 255:
                                            pass
                                        else:
                                            DLL_API_matrix[y][x] += 1
...
def train_FastText(FastText_path, name='OPCODE', total_exam=500):
    ...
    FT64_model.train(sentences, total_examples=total_exam, epochs=20)
    ...
    FT128_model.train(sentences, total_examples=total_exam, epochs=20)
    ...
    FT256_model.train(sentences, total_examples=total_exam, epochs=20)
    ...
    FT512_model.train(sentences, total_examples=total_exam, epochs=20)
...
if __name__ == '__main__':
    ...
    # ASM_SRC_PATH = 'C://Users/Ucloud/Downloads/malware-classification/test'
    BYTES_SRC_PATH = 'C://Users/Ucloud/Downloads/malware-classification/bytes_files'
    LABEL_FILE = './BIGSet/trainLabels.csv'
    TRAIN_FEATURE_PATH = './BIGSet/features'

    # OPCODE_SET_PATH = os.path.join(TRAIN_FEATURE_PATH, 'opcode')
    TOTAL_OPCODE_SET_PATH = os.path.join('./TotalSet', 'opcode')
    # API_SET_PATH = os.path.join(TRAIN_FEATURE_PATH, 'Api')
    TOTAL_API_SET_PATH = os.path.join('./TotalSet', 'API')
    DLL_API_SET_PATH = os.path.join(TRAIN_FEATURE_PATH, 'DllApi')
    # FASTTEXT_BASE_DIR = './dataset/FastText'
    TOTAL_FASTTEXT_BASE_DIR = os.path.join('./TotalSet', 'FastText')
    ...
    TOTAL_OPCODE_IMG_PATH = os.path.join('./TotalSet', 'opcodeImg')
    TOTAL_API_IMG_PATH = os.path.join('./TotalSet', 'ApiImg')
    ...
    #
    # Generate_FastText_TrainSet(TOTAL_OPCODE_SET_PATH, TOTAL_FASTTEXT_BASE_DIR, data_type='OPCODE')
    # label_del(TOTAL_FASTTEXT_BASE_DIR, name='OPCODE')
    # Generate_FastText_TrainSet(TOTAL_API_SET_PATH, TOTAL_FASTTEXT_BASE_DIR, data_type='API')
    # label_del(TOTAL_FASTTEXT_BASE_DIR, name='API')
    #
    # train_FastText(TOTAL_FASTTEXT_BASE_DIR, name='OPCODE', total_exam=12076)
    # train_FastText(TOTAL_FASTTEXT_BASE_DIR, name='API', total_exam=12076)
    #
    # embedding_list = [64, 128, 256, 512]
    # for size in embedding_list:
    #     GenerateFastTextImg(TOTAL_FASTTEXT_BASE_DIR, TOTAL_OPCODE_IMG_PATH, name='OPCODE', embedding_size=size)
    #     GenerateFastTextImg(TOTAL_FASTTEXT_BASE_DIR, TOTAL_API_IMG_PATH, name='API', embedding_size=size)